...
export class UnifiedRecommendationsComponent implements OnInit {
    ...
    loadData() {
        ...
        this.servicesService.getServices().subscribe(services => {
            ...
            if (this.type === 'movie' && radarr) {
                this.radarrService.getRecommendations(radarr.id!).pipe(catchError(() => of([]))).subscribe((data: any[]) => {
                    this.serviceRecs.set(data);
                    this.checkLoading();
                });
            ...
            if (this.type === 'show' && sonarr) {
                this.sonarrService.getRecommendations(sonarr.id!).pipe(catchError(() => of([]))).subscribe((data: any[]) => {
                    this.serviceRecs.set(data);
                    this.checkLoading();
                });
            ...
            if (!trakt) {
                if (this.type === 'movie' && !radarr) this.loading.set(false);
                if (this.type === 'show' && !sonarr) this.loading.set(false);
            }
        });
    }
    ...
    mapToMediaItem(item: UnifiedItem): MediaItem {
        ...
        return {
            ...
            imageUrl: this.ensureProtocol(item.poster),
            sourceIcons: icons,
            clickAction: () => {
