...
export class SonarrController {
    ...
    getQueue(@Param('id') id: string) {
        return this.sonarrService.getQueue(+id);
    }

    @Get(':id/diskspace')
    getDiskSpace(@Param('id') id: string) {
        return this.sonarrService.getDiskSpace(+id);
    }

    @Get(':id/episode')
    getEpisodes(@Param('id') id: string, @Query('seriesId') seriesId: string) {
        return this.sonarrService.getEpisodes(+id, +seriesId);
    }
}
