...
export class TraktController {
    ...
    @Get(':id/shows/:traktId/seasons/:seasonNumber')
    getSeasonEpisodes(
        @Param('id') id: string,
        @Param('traktId') traktId: string,
        @Param('seasonNumber') seasonNumber: string
    ) {
        return this.traktService.getSeasonEpisodes(+id, +traktId, +seasonNumber);
    }
    ...
    async searchQuery(
        ...
        return this.traktService.searchQuery(id, query, type);
    }
}
